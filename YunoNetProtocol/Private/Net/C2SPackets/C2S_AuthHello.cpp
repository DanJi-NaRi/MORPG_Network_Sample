...
namespace yuno::net::packets
{
    namespace
    {
        void WriteStringU16(ByteWriter& w, const std::string& value)
        {
            if (value.size() > static_cast<std::size_t>(std::numeric_limits<std::uint16_t>::max()))
                throw std::runtime_error("C2S_AuthHello string too long");
        ...
        std::string ReadStringU16(ByteReader& r)
        {
            ...
            if (!r.Has(length))
                throw std::runtime_error("C2S_AuthHello invalid string length");
    ...
    void C2S_AuthHello::Serialize(ByteWriter& w) const
    {
        WriteStringU16(w, loginId);
        WriteStringU16(w, password);
    }

    C2S_AuthHello C2S_AuthHello::Deserialize(ByteReader& r)
    {
        C2S_AuthHello out;
